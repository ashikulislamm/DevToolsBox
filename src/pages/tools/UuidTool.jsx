...
export default function UuidTool() {
  ...
  const copyUUID = async () => {
    if (!uuid.trim()) {
      showNotification(
        "No UUID to copy! Please generate a UUID first.",
        "error"
      );
      return;
    }
    ...
    try {
      ...
      showNotification("📋 UUID copied to clipboard successfully!", "success");
    } catch (err) {
      showNotification("❌ Failed to copy UUID to clipboard", "error");
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
        <h1 className="md:text-2xl font-bold text-white">🆔 UUID Generator</h1>
      ...
      <div className="flex flex-wrap gap-4 mb-4">
        <div>
          <label className="text-sm text-gray-300 mr-2">UUID Version:</label>
          <select
            value={version}
            onChange={(e) => setVersion(e.target.value)}
            className="px-2 py-1 rounded-md bg-gray-900 border border-gray-600 text-sm outline-none"
          >
        ...
        <div>
          <label className="text-sm text-gray-300 mr-2">Count:</label>
          <input
            type="number"
            min="1"
            max="50"
            ...
            className="w-20 px-2 py-1 rounded-md bg-gray-900 border border-gray-600 text-sm outline-none"
          />
        </div>
      </div>
      ...
      <textarea
        value={uuid}
        readOnly
        className="w-full h-64 p-3 border rounded-lg font-mono text-sm bg-gray-900 text-gray-100 resize-none custom-scrollbar"
        style={{
          borderColor: "var(--secondary-color)",
          fontFamily: "var(--font-family)",
        }}
