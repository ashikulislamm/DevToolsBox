import{r as i,u as P,j as e,m as R,e as G,f as U,n as B,h as _}from"./index-SRiEpqb_.js";import{C as A,a as O,B as u}from"./Button-BlCWr7aF.js";import{I as F}from"./Input-DGWi4jVI.js";import{T as C}from"./Textarea-DplyL_uy.js";import{u as J}from"./useClipboard-BzP020Xg.js";function W(){const[r,q]=i.useState("GET"),[h,x]=i.useState(""),[j,g]=i.useState(""),[l,f]=i.useState(""),[$,T]=i.useState("fetch"),[p,b]=i.useState(""),[m,E]=i.useState(!1),{showToast:d}=P(),{copy:N}=J(),k=t=>{if(!t.trim())return{"Content-Type":"application/json"};const n=t.split(`
`).filter(s=>s.includes(":")),c={};return n.forEach(s=>{const[o,...a]=s.split(":"),v=o.trim(),S=a.join(":").trim();v&&S&&(c[v]=S)}),c},y=()=>{if(!h.trim()){d("❌ Please enter a valid URL.","error");return}let t=h.trim();!t.startsWith("http://")&&!t.startsWith("https://")&&(t="https://"+t,x(t));let n;try{n=k(j)}catch{d("❌ Failed to parse headers. Ensure key: value format.","error");return}const c=JSON.stringify(n,null,2);let s="";switch($){case"fetch":s=`fetch("${t}", {
  method: "${r}",
  headers: ${c},
  body: ${r==="GET"?"undefined":`JSON.stringify(${l||"{}"})`}
})
  .then(res => {
    if (!res.ok) {
...
${r!=="GET"?`req.write(JSON.stringify(${l||"{}"}));`:""}
